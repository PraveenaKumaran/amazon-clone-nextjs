...
const page = () =>{
  ...
  const handleSingup = async () => {
    if(email && password) {
      ...
      // Wait for the response from the signup function
      if(response.status == 201){
        // If the signup is successful, reads user info from the boby of response given by the server, then store it in user and redirect to the login page
        router.push("/login");
      }
    }
  }
